...
metadata {
	definition (name: "Wiser Zigbee iTRV - Update v0.2", namespace: "gszabados", author: "gszabados") {
	...
	tiles(scale : 2) {
		multiAttributeTile(name:"thermostatMulti", type:"thermostat", width:6, height:4, icon:"st.Home.home1", canChangeIcon: true) {
            ...
            tileAttribute("device.nextHeatingSetpoint", key: "VALUE_CONTROL", label:'${currentValue}°') {
	...
	if (value != null)
	{
		...
		if (radiatorTemperature != currentTemperature)
		{
			...
			else
			{
				...
				sendEvent(name:"nextHeatingSetpoint", value: radiatorTemperature, unit: getTemperatureScale(), displayed: false)
			}
...
def setValveHeatingSetpoint() {
	...
	//{
		...
		if (nextHeatingSetpoint != 0 && nextHeatingSetpoint != heatingSetpoint)
		{
			...
			def cmds =
					zigbee.writeAttribute(0x201, 0x12, 0x29, hex(nextHeatingSetpoint * 100)) //+
...
def increaseHeatSetpoint() {
    ...
    if (currentMode != "off")
    {
		float currentSetpoint = device.currentValue("nextHeatingSetpoint")
		float maxSetpoint
		float step
...
def updated() {
	...
	*/
	...
	def radiatorTemperature = currentDouble("heatingSetpoint")
	def nextTemperature = currentDouble("nextHeatingSetpoint")
	...
	if(nextTemperature == 0)
	{
		...
		state.lastSentTemperature = radiatorTemperature
		sendEvent(name:"nextHeatingSetpoint", value: radiatorTemperature, unit: getTemperatureScale(), displayed: true)
	}
...
private List<Map> parseComplexValues(attrData) {
    ...
    def setPointEvent = createEvent(name: "heatingSetpoint", value: heatSetPoint)

...
/* 
Originally Created by SmartThings
Enhanced with Mutable DTH Modifications
By Austin J. Pritchett
@ajpri
...
*/
metadata {
	definition (name: "Mutable Z-Wave Metering Switch", namespace: "ajpri", author: "ajpri", ocfDeviceType: "oic.d.switch") {
    ...
    preferences {
		section {
			input "logPower", "bool", title: "Show Power in Activity Feed" , description: "Show Power in feed.", displayDuringSetup: false 
        }
	}
	// tile definitions
	tiles(scale: 2) {
...
def zwaveEvent(physicalgraph.zwave.commands.meterv1.MeterReport cmd) {
	if (cmd.scale == 0) {
		createEvent(name: "energy", value: cmd.scaledMeterValue, unit: "kWh", displayed: logPower)
	} else if (cmd.scale == 1) {
		createEvent(name: "energy", value: cmd.scaledMeterValue, unit: "kVAh", displayed: logPower)
	} else if (cmd.scale == 2) {
		createEvent(name: "power", value: Math.round(cmd.scaledMeterValue), unit: "W", displayed: logPower)
	}
}
